...
- [x] **Наполнены 4 кабинета** (канон через MCP + встречи): Куратор (blue), Ученик (green), Родитель (blue), Финансист (gold). Каждый — ядро-процесс, 13–15 доменов, связи, модули, `sources[]`
...
- [x] **Указание владельца:** доля распределения роялти «50% Довлатову» не отображается в атласе (см. memory)
- [x] **Push + production-деплой** (06.06): ветка в `origin`; `vercel deploy --prod` → **live `samoarchitecture.vercel.app`** (18 страниц, проверено)
- [x] Проверки: typecheck 0 · **test 69/69** · build 18 страниц · **e2e 11/11** (golden'ы L0 + 4 кабинета) · vision gate 2 vs эталонные PNG

**Не сделано / блокеры:**
- [ ] Кабинеты, кроме Куратора/Ученика/Родителя/Финансиста, — навигируемые стабы (rollout — следующие фазы)
