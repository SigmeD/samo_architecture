...
**Текущая фаза.** v0 — дизайн-спека и план утверждены; реализуется вертикальный срез «Куратор». Код приложения ещё не написан (репо пока docs-only).
...
**Сделано:**
- [x] Дизайн-спека утверждена (`docs/superpowers/specs/2026-06-06-...`) — 19 фактов верифицированы против канона
- [x] План вертикального среза «Куратор» утверждён (`docs/superpowers/plans/2026-06-06-...`)
- [x] Инженерная архитектура атласа (`docs/architecture/frontend-architecture.md`) — заменила платформенную (защитный коммит `f134113`, замена `0ea981d`)
- [x] Брендбук + эталонные постеры (11 кабинетов + обзор) на месте
- [x] `.mcp.json` (канон) подключён, токен в `.gitignore`

**Не сделано / блокеры:**
- [ ] Код приложения ещё не написан (репо docs-only) — Фаза 1 Task 1: каркас Next.js
- [ ] `docs/STATUS.md` не заведён (Фаза 0 Task 0)
- [ ] Вертикальный срез «Куратор»: постер → кабинет (15 доменов) → 2 модуля; остальные 10 кабинетов — навигируемые стабы
- [ ] Playwright screenshot-diff против `dnm-cabinet-curator.png`

**Открытые вопросы** (показываются в атласе как «на подтверждении», полный список — спека §12):
- Финальный пакет процедуры урока (3-фаза `REG-DNM-LESSON-001` vs 9-этапная методичка `REG-DNM-CURATOR-001`) — Айгерим (br7)
- Номиналы соларов и лимит за семестр (~1190 vs ~1140) — расхождение канона
- Фикс-ставки и планы KPI для расчёта выплат; связь рейтинга куратора с выплатами
- Функции ИИ-аватара Довлатова в кабинете куратора
