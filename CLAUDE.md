...
**Текущая фаза.** v0 — приложение собрано и задеплоено (`samoarchitecture.vercel.app`). L0-обзор и общий рендерер кабинетов переведены в единую постер-стилистику (ветка `feature/poster-style-redesign`); наполнены кабинеты **Куратора**, **Ученика**, **Родителя** и **Финансиста/бухгалтера** (канон + встречи 04–05.06). Идёт rollout остальных кабинетов из стабов (план `docs/superpowers/plans/2026-06-06-cabinets-poster-rollout.md`).
...
- Зона красит только акцент-полосу/бордеры/бейджи; фоны — белые/синие. Для текста <18px — тёмные варианты акцентов (`--color-samo-orange-d` / `--color-samo-green-d`, контраст WCAG AA).
...
- [x] **L0 постер-редизайн** (`feature/poster-style-redesign`): дизайн-система постер-стиля (9 компонентов), `OverviewSpec`, контент по встречам 04–05.06 — 3 этажа / 12 кабинетов (добавлен **Маркетолог**; «Качество» — функция админа, не кабинет); golden `overview-l0.png`; vision-first-ui gate 2 пройден
- [x] Проверки: typecheck 0 · test 30/30 · build 18 страниц · e2e 5/5

**Не сделано / блокеры:**
- [ ] Кабинеты, кроме Куратора, Ученика, Родителя и Финансиста, — навигируемые стабы (rollout — следующие фазы)
- [x] Канон синхронизирован со встречами 04–05.06 (сверка 06.06): Родитель/Маркетолог/«отдел качества»=функция админа/ИИ-аватар внесены в канон (`CONV-ROLES-DNM-001 v1.2`, `CONV-ROLE-HIERARCHY-001 v1.6`, `SPEC-DNM-TZ-001 v3.3`, `REG-DNM-SCHOOL-ADMIN-001 v1.1`). Код `op-marketolog-dnm` — теперь официальный канон. `propose_change` не нужен
- [ ] Финал: code-review L0 → merge `feature/poster-style-redesign` → деплой preview

**Открытые вопросы** (показываются в атласе как «на подтверждении», полный список — спека §12, канон-сверка — `docs/STATUS.md`):
- Процедура урока: 3-фаза `REG-DNM-LESSON-001` vs 9-этапная методичка `REG-DNM-CURATOR-001` — Айгерим (br7)
...
- Функции ИИ-аватара Довлатова в кабинетах ученика/родителя/куратора (канон относит к отд. продукту M19)
