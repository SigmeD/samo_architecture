...
## Текущий статус (2026-06-09)

> Подробное зеркало кода — `docs/STATUS.md`. Здесь — срез верхнего уровня.

**Сделано:**
- [x] **Итог 09.06 (`update_09_06` → `main`, PR #7 `4a33c6f` + PR #8 `a4e1381`, прод live):** 13 кабинетов (+методист); рендер связей обоюдные/односторонние + **stub-связь** (`CrossLink.stub` — роль без кабинета, напр. заглушка «Финансист ГО» Само Глобал); **строгая вертикаль данных/эскалации** + гард-тест `system-graph.test.ts`; **чат-хаб админа школы**; **реверс воронки продаж** (договор у менеджера = канон RBAC v1.4 §6, canon-proposals sales-S2/guest отозваны); финлиния ГО (Руководитель ↔ Финансист ГО заглушкой, не школьный бухгалтер); чистка `/map`; ДЗ-статусы RU + соляры/значки куратора; «Менеджер по продажам ДНМ» / «Старший куратор ДНМ»; **метки New убраны ВЕЗДЕ** (отменяет «оставить на проде»). typecheck 0 · test 248/248 · e2e 31/31 · граф both-консистентен · vision-gate 2 ✓
...
- [x] **Кабинет гостя** (`guest.ts`, 08.06, ветка `update_08_06`) — **ПОСЛЕДНИЙ из 12; реестр `CABINETS` наполнен целиком, стабов нет** (хелпер `stub()` удалён). Из эталон-постера `dnm-cabinet-guest.{html,png}` (02.06) + встреча 04.06 + канон. Роль `cr-gost-dnm`, **зона green**, ядро «Онбординг — от знакомства до зачисления» (6 шагов), 8 доменов, 7 связей, 7 модулей. **Поправки к постеру (метки New):** пробное проводит КУРАТОР (не менеджер); договор/оплату/рассрочку/зачисление ведёт АДМИНИСТРАТОР + финансы, факт оплаты — БУХГАЛТЕР; вход телефон+код (не email/Google). **Инварианты:** гость = состояние до авторизации (не роль RBAC, не строка `/map`); изолированная роль (ADR-003); реф-регистрация ребёнка — право только родителя; 7-дневный TTL; доступ к урокам только после оферты И `payment.confirmed`. **Расхождения → `docs/canon-proposals/2026-06-08-guest-cabinet.md`** (главное — само-противоречие канона «пробное=куратор vs менеджер», синхронизировать под TZ v3.8; оплата CRU у Мен убрать). Guardrails: обезличено, без доли 50%. typecheck 0 · **test 160/160** (guest +12) · build 19 · **e2e 29/29** (golden `cabinet-guest.png`) · vision gate 2 ✓.
- [x] **Выкат на прод (08.06):** `update_08_06` **смёржен в `main`** (PR #2, merge `b8d7101`) → live `samoarchitecture.vercel.app` (проверено: `/map` + кабинет руководителя). ⚠️ Метки **New оставлены на проде** по решению владельца (для показа клиента) — снятие отдельным коммитом позже (см. `docs/STATUS.md`).
- [x] **Редизайн кабинета ученика** (`child.ts`, 09.06, ветка `update_08_06`) — наложение приоритетов **удержание · посещение · честная механика баллов** + когортный слой JUNIOR/SENIOR. **Доменов 13 → 16** (split Эссе/Бизнес-проект; +«Когортный слой», +«Удержание и посещение»). Ключевое: солары **merit/gift** (merit→рейтинг lifetime, gift→только баланс), merit-stretch **сверх потолка 1190**; рейтинг = Версия A в **3 среза × когорту** (группа·школа·сеть × JUNIOR/SENIOR×семестр), подача **тиры/перцентиль**; посещение — раннее предупреждение/наставник/мягкий возврат; чат одноклассников. Исполнено **subagent-driven** (implementer → spec-review → code-quality; vision-gate 2 контроллером). **Метки New на 8 переработанных доменах.** Расхождения → `docs/canon-proposals/2026-06-09-uchenik-redesign.md` (U1–U8, на согласовании; канон НЕ правился). Спека/план — `docs/superpowers/specs|plans/2026-06-09-*`. typecheck 0 · **test 167/167** · build 19 · **e2e 29/29** · vision gate 2 ✓. **PR #4 (редизайн) и PR #5 (метки New) смёржены в `main` → прод-деплои verified.**
- [x] **MD-описания 12 кабинетов** (`docs/cabinets/*.md`, 09.06) — производные человекочитаемые описания от `content/cabinets/*.ts` (источник правды), латинские имена. Фан-аут субагентов + верификация счёта против источника (5 опечаток в заголовках исправлены; заодно фикс guest 8→7 модулей в CLAUDE/STATUS).
