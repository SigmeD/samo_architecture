...
- [x] **Наполнены 7 кабинетов** (канон + встречи + проектный док): Куратор (blue), Ученик (green), Родитель (blue), Финансист (gold), **Куратор франшиз (purple, 14 доменов)**, **Франчайзи/директор (purple, 13 доменов — владелец сети)**, **Администратор школы (orange, 14 доменов — операционный центр школы; ядро «Зачисление-мастер»; граница: только дебиторка, не P&L/роялти; 07.06, Workflow ultracode + Google Doc через Drive MCP)**. Каждый — ядро-процесс, домены, связи, модули, `sources[]`
...
**Не сделано / блокеры:**
- [ ] Кабинеты-стабы (rollout — следующие фазы): senior-curator, sales, marketer, lead, guest (готовы: Куратор/Ученик/Родитель/Финансист/Куратор франшиз/Франчайзи/**Администратор школы**)
