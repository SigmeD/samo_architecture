...
test("кабинет куратора франшиз: постер-рендер, purple-зона, обезличено, доступы к ЛК", async ({ page }) => {
  ...
  // обоюдная связь (франчайзи) кликабельна — КФ курирует франчайзи
  await page.getByRole("link", { name: /Франчайзи/ }).first().click();
  await expect(page).toHaveURL(/\/cabinet\/franchise/);
});
...
test("кабинет франчайзи-партнёра: постер-рендер, purple-зона, финансы+планирование, обезличено", async ({ page }) => {
  await page.goto("/cabinet/franchise/");
  await expect(page.getByRole("heading", { level: 1, name: /Франчайзи-партнёр/ })).toBeVisible();
  await expect(page.getByRole("heading", { level: 2, name: "Домены и функции" })).toBeVisible();
...
test("кабинет франчайзи-партнёра: визуальный снимок (baseline)", async ({ page }) => {
  await page.goto("/cabinet/franchise/");
  await expect(page).toHaveScreenshot("cabinet-franchise.png", { fullPage: true });
});

test("кабинет администратора школы (лёгкий): постер-рендер, orange-зона, граница финансов, Лидоруб, обезличено", async ({ page }) => {
  await page.goto("/cabinet/school-admin/");
  await expect(page.getByRole("heading", { level: 1, name: /Администратор школы/ })).toBeVisible();
  await expect(page.getByRole("heading", { level: 2, name: "Домены и функции" })).toBeVisible();
  // лёгкий кабинет: ядро — «Приём и распределение»; граница финансов (P&L закрыт); dual-mode Лидоруб; обезличено
  await expect(page.getByText(/Приём и распределение/).first()).toBeVisible();
  await expect(page.getByText(/только владельцу и бухгалтеру/).first()).toBeVisible();
  await expect(page.getByText(/Лидоруб/).first()).toBeVisible();
  await expect(page.locator("body")).not.toContainText("50%");
  await expect(page.locator("body")).not.toContainText(/Д[оа]влат/);
...
test("кабинет старшего куратора: постер-рендер, blue-зона, двойная роль, обезличено", async ({ page }) => {
  ...
  await expect(page.locator("body")).not.toContainText("50%");
  await expect(page.locator("body")).not.toContainText(/Д[оа]влат/);
  // обоюдная связь (администратор школы) кликабельна — СК передаёт отчёты админу
...
test("кабинет руководителя проекта: blue-зона, стратегический контур, прямая связь с Финансист ГО, обезличено", async ({ page }) => {
  ...
  // метки New убраны
  await expect(page.locator('[data-new="true"]')).toHaveCount(0);
  // прямая связь с Финансист ГО (Само Глобал) — видимая обоюдная карточка, не односторонний блок
  await expect(page.getByText(/Финансист ГО/).first()).toBeVisible();
...
test("кабинет менеджера по продажам ДНМ: teal-зона, воронка лида, реверс (менеджер оформляет договор), обезличено", async ({ page }) => {
  await page.goto("/cabinet/sales/");
  await expect(page.getByRole("heading", { level: 1, name: /Менеджер по продажам ДНМ/ })).toBeVisible();
  await expect(page.getByRole("heading", { level: 2, name: "Домены и функции" })).toBeVisible();
  // ядро — воронка лида-родителя; реверс: договор оформляет менеджер, админ подтверждает
  await expect(page.getByText(/Воронка лида-родителя/).first()).toBeVisible();
  await expect(page.getByText(/менеджер.*оформляет договор|договор.*оформляет менеджер|админ\S*.*подтвержда/i).first()).toBeVisible();
  // метки New убраны
  await expect(page.locator('[data-new="true"]')).toHaveCount(0);
  // инварианты: без доли роялти, без имён
  await expect(page.locator("body")).not.toContainText("50%");
  await expect(page.locator("body")).not.toContainText(/Д[оа]влат/);
  // обоюдная связь (маркетолог) кликабельна; администратор школы — односторонняя (текстом ниже)
  await page.getByRole("link", { name: /Маркетолог/ }).first().click();
  await expect(page).toHaveURL(/\/cabinet\/marketer/);
});
...
test("кабинет маркетолога: orange-зона, контур атрибуции, двухуровневая модель, связка с продажами, обезличено", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { level: 2, name: "Домены и функции" })).toBeVisible();
  // ядро — лидогенерация → квалифицированный лид («ноль финансов»); двухуровневая модель
  await expect(page.getByText(/квалифицированн|лидогенерации/i).first()).toBeVisible();
  await expect(page.getByText(/двухуровнев/i).first()).toBeVisible();
  // C2: маркетолог НЕ видит выручку/close-rate (это ГО) — «ноль финансов»
  await expect(page.getByText(/ноль финансов/i).first()).toBeVisible();
...
test("кабинет гостя: green-зона, онбординг, реверс (менеджер ведёт пробное/договор), изолированная роль, обезличено", async ({ page }) => {
  await page.goto("/cabinet/guest/");
  await expect(page.getByRole("heading", { level: 1, name: /Гость/ })).toBeVisible();
  await expect(page.getByRole("heading", { level: 2, name: "Домены и функции" })).toBeVisible();
  // ядро — онбординг; C1 закрыт: менеджер ведёт пробное сам ИЛИ привлекает куратора по цепочке менеджер→админ→ст.куратор, оформляет договор
  await expect(page.getByText(/Онбординг/).first()).toBeVisible();
  await expect(page.getByText(/привлекает куратора|через ст\S*\s*куратор|менеджер.*провод\S*.*пробн|менеджер.*оформляет договор/i).first()).toBeVisible();
  // метки New убраны
  await expect(page.locator('[data-new="true"]')).toHaveCount(0);
  // инварианты: без доли роялти, без имён
  await expect(page.locator("body")).not.toContainText("50%");
  await expect(page.locator("body")).not.toContainText(/Д[оа]влат/);
  // обоюдная связь (менеджер по продажам) кликабельна; куратор — односторонняя (текстом ниже)
  await page.getByRole("link", { name: /Менеджер по продажам/ }).first().click();
  await expect(page).toHaveURL(/\/cabinet\/sales/);
});
