...
test("кабинет гостя: green-зона, онбординг, реверс (менеджер ведёт пробное/договор), изолированная роль, обезличено", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { level: 2, name: "Домены и функции" })).toBeVisible();
  // ядро — онбординг; реверс: менеджер ведёт пробное сам или назначает куратора, оформляет договор
  await expect(page.getByText(/Онбординг/).first()).toBeVisible();
  await expect(page.getByText(/менеджер.*провод\S*\s+пробн|назначает куратора|менеджер.*оформляет договор/i).first()).toBeVisible();
  // метки New убраны
  await expect(page.locator('[data-new="true"]')).toHaveCount(0);
  // инварианты: без доли роялти, без имён
  await expect(page.locator("body")).not.toContainText("50%");
