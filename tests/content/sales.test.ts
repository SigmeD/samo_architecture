...
describe("кабинет менеджера по продажам (sales) — реверс 09.06", () => {
  ...
  it("C1 закрыт: менеджер ведёт пробное сам ИЛИ привлекает куратора по цепочке менеджер→администратор→ст.куратор", () => {
    const blob = JSON.stringify(sales);
    expect(blob).toMatch(/менеджер проводит[^"]*сам|проводит онбординг[^"]*сам|менеджер ведёт[^"]*сам/i);
    // цепочка привлечения куратора через администратора → старшего куратора
    expect(blob).toMatch(/администратор\S*\s*→\s*стар\S*\s*куратор|администратор\S*[^"]*стар\S*\s*куратор/i);
    expect(blob).toMatch(/видеофиксац|контрол\S*[^"]*качеств/i);
    expect(blob).toMatch(/close-rate|close rate/i);
    // C1 закрыт, обе модели валидны — нет «на согласовании»
    expect(blob).toMatch(/C1 закрыт/i);
    expect(blob).not.toMatch(/на согласовании \(C1/i);
    expect(blob).not.toMatch(/не ратифициров/i);
    // менеджер НЕ назначает куратора напрямую (цепочка через администратора → ст.куратора)
    expect(blob).toMatch(/менеджер не назначает куратора напрямую|менеджер не назначает напрямую/i);
  ...
  it("crossLink curator: пробное менеджер сам ИЛИ куратор по цепочке менеджер→админ→ст.куратор, под контролем качества", () => {
    ...
    // куратор привлекается по цепочке через администратора → старшего куратора
    expect(cur?.label).toMatch(/администратор\S*\s*→\s*стар\S*\s*куратор|администратор\S*[^"]*стар\S*\s*куратор/i);
    expect(cur?.label).toMatch(/контрол\S*[^"]*качеств|видеофиксац/i);
    expect(cur?.label).toMatch(/C1 закрыт/i);
  });
