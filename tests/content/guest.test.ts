...
describe("кабинет гостя (guest) — 08.06, последний из 12", () => {
  ...
  it("C1 закрыт: менеджер ведёт онбординг и пробное сам ИЛИ привлекает куратора по цепочке менеджер→админ→ст.куратор", () => {
    const blob = JSON.stringify(guest);
  ...
  it("постер 02.06 верен (логика менеджера/договора подтверждена каноном); нет меток New", () => {
    const blob = JSON.stringify(guest);
    // менеджер ведёт пробное (сам) ИЛИ привлекает куратора по цепочке — обе модели валидны (C1 закрыт)
    expect(blob).toMatch(/менеджер[^"]*(ведёт|провод\S*)[^"]*пробн|пробн\S*[^"]*(ведёт|провод\S*)[^"]*менеджер|привлекает куратора/i);
